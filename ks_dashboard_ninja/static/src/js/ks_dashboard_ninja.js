...
//                result = self.normalize_dn_data(result);
        ...
        onKsDashboardMenuContainerShow: function(e) {
            ...
            if (!this.ks_dashboard_data.ks_item_data[item_id].ks_model_id){
                e.preventDefault();
                return
            }
...
//                            framework.unblockUI();
                ...
                if (e.target.title != "Customize Item") {
                    ...
                    if (item_data && item_data.ks_show_records) {
                        ...
                        if (item_data.action) {
                            ...
                            if (item_data.ks_dashboard_item_type == "ks_kpi" && item_data.ks_model_id_2 && (item_data.ks_data_comparison == "Ratio" || item_data.ks_data_comparison == "None")){
                                Dialog.confirm(this, (_t("Which model records would you be excited to check out?")), {
                                    buttons: [
                                        {
                                            text: _t("Data 1"),
                                            classes: 'btn-primary',
                                            close: true,
                                            click: function(ev) {
                                                self.ks_confirm_callback(ev,self.ks_item_data,false);
                                            }
                                        }, {
                                            text: _t("Data 2"),
                                            close:true,
                                            click: function(ev){
                                                self.ks_confirm_callback(ev,self.ks_item_data,true)
                                            }
                                        }
                                    ],
                                });
        ...
        ks_confirm_callback: function(ev,item_data,confirm) {
            var self = this;
            let ks_model_data = (JSON.parse(item_data.ks_kpi_data))[1]
            let model = confirm? ks_model_data.model :item_data.ks_model_name
            let domain = confirm? item_data.ks_domain_2 : item_data.ks_domain
            var action = {
                name: _t(item_data.name),
                type: 'ir.actions.act_window',
                res_model:model,
...
//                self.ksRenderDashboard();
        ...
        _renderDateFilterDatePicker: function() {
            ...
            if(self.ks_dashboard_data.ks_day_end_time){
                var endDate = self.ks_dashboard_data.ks_dashboard_end_date ? moment.utc(self.ks_dashboard_data.ks_dashboard_end_date).local() : moment().endOf('day')  ;
            }else{
